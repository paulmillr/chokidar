...
function depth(path, parent) {
  var i = 0;
  while (
    !path.indexOf(parent) && (path = sysPath.dirname(path)) !== parent
  ) i++;
  return i;
}
...
function(path, pathTransform, forceScan, priorDepth) {
    ...
    fs[followSymlinks ? 'stat' : 'lstat'](path, function(error, stats) {
      ...
      if (stats.isDirectory()) {
        ...
        readdirp({
          ...
          var entryPath = sysPath.join(path, entry.path);
          ...
          if (followSymlinks && entry.stat.isSymbolicLink()) {
            ...
            var curDepth = this.options.depth === undefined ?
              undefined : depth(entryPath, sysPath.resolve(path)) + 1;
  ...
  if (this.options.persistent) {
    if (typeof pathTransform === 'function') {
      ...
      this._watchWithFsEvents(path, sysPath.resolve(path), processPath);
    } else {
      fs.realpath(path, function(error, realPath) {
        if (error) realPath = path;
        this._watchWithFsEvents(path, sysPath.resolve(realPath), processPath);
      }.bind(this));
    }
  }
  return this;
