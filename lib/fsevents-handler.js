...
function setFSEventsListener(path, realPath, listener, rawEmitter) {
  // console.log('setFsEventsListener', path);
  ...
  if (cont || watchedParent()) {
    cont.listeners.push(filteredListener);
  } else {
    cont = {
      ...
      watcher: createFSEventsInstance(watchPath, (fullPath, flags) => {
        ...
        // console.log('listener', fullPath, cont.listeners);
        cont.listeners.forEach(cb => {
          cb(fullPath, flags, info);
        });
  ...
  const index = wl.indexOf(filteredListener);
...
_addToFsEvents(path, transform, forceAdd, priorDepth) {
  ...
  fs[wh.statMethod](wh.watchPath,
    ...
    if (stats.isDirectory()) {
      ...
      }).on('error', () => {/* Ignore readdirp errors */}).on('end', this.fsw._emitReady);
    } else {
