...
function couldConsolidate(path) {
  const keys = Object.keys(FSEventsWatchers);
  let count = 0;
...
const canUse = () => fsevents && Object.keys(FSEventsWatchers).length < 128;
...
_watchWithFsEvents(watchPath, realPath, transform, globFilter) {
  ...
  const watchCallback = function watchCallback(fullPath, flags, info) {
    if (
    ...
    const checkIgnored = function checkIgnored(stats) {
...
_addToFsEvents(path, transform, forceAdd, priorDepth) {
  ...
  const emitAdd = function(newPath, stats) {
  ...
  fs[wh.statMethod](wh.watchPath, function(error, stats) {
    ...
    if (stats.isDirectory()) {
      ...
      }.bind(this)).on('error', () => {/* Ignore readdirp errors */}).on('end', this._emitReady);
    } else {
  ...
  if (this.options.persistent && forceAdd !== true) {
    const initWatch = function(error, realPath) {
