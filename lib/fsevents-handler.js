...
function(linkPath, pathTransform, depth) {
  this._readyCount++;
...
function(path, pathTransform, forceScan, priorDepth) {
  ...
  var globstarParts = path.split('**');
  var globstarRoot = globstarParts.length > 1 && globstarParts[0];
  var globstarFilter = function(entry) {
    var entryPath = sysPath.join(watchPath, entry.path);
    return globstarRoot &&
      entry.stat.isDirectory() &&
      // beginning parts of both paths match
      !sysPath.resolve(entryPath).indexOf(sysPath.resolve(globstarRoot))
  }
  var followSymlinks = this.options.followSymlinks;
  var filterPath = function(entry) {
    var entryPath = sysPath.join(watchPath, entry.path);
    return (!globFilter || globFilter(entryPath)) &&
      this._isntIgnored(entryPath, entry.stat);
  }.bind(this);
  var filterDir = function(entry) {
    return globstarFilter(entry) || filterPath(entry);
  }
  fs[followSymlinks ? 'stat' : 'lstat'](watchPath, function(error, stats) {
    if (this._handleError(error)) return this._emitReady();

    if (stats.isDirectory()) {
      if (!globFilter) emitAdd(processPath(path), stats);
      if (priorDepth && priorDepth > this.options.depth) return;
      readdirp({
        root: watchPath,
        entryType: 'all',
        fileFilter: filterPath,
        directoryFilter: filterDir,
        lstat: true,
        depth: this.options.depth - (priorDepth || 0)
      }).on('data', function(entry) {
        if (!filterPath(entry)) return;
        var entryPath = sysPath.join(watchPath, entry.path);
        ...
        if (followSymlinks && entry.stat.isSymbolicLink()) {
          ...
          var curDepth = this.options.depth === undefined ?
            undefined : depth(entryPath, sysPath.resolve(watchPath)) + 1;
          this._handleSymlinkForFsEvents(entryPath, processPath, curDepth);
        } else {
          addEntry();
        }
      }.bind(this)).on('end', this._emitReady);
    } else {
      emitAdd(watchPath, stats);
      this._emitReady();
    }
  }.bind(this));
  if (this.options.persistent) {
    if (typeof pathTransform === 'function') {
      ...
      this._watchWithFsEvents(
        watchPath,
        sysPath.resolve(watchPath),
        processPath,
        globFilter
      );
    } else {
      fs.realpath(watchPath, function(error, realPath) {
        if (error) realPath = watchPath;
        this._watchWithFsEvents(
          watchPath,
          sysPath.resolve(realPath),
          processPath,
          globFilter
        );
      }.bind(this));
