...
class FSWatcher extends EventEmitter {
  ...
  */
  ...
  _throttle(Action, ThePath, Timeout){
  ...
  _getWatchHelpers(ThePath, Depth){
  ...
  _remove(Directory, ItemPath){
    var
      RelativePath = Path.join(Directory, ItemPath),
      FullPath = Path.resolve(RelativePath),
      isDirectory = this._watched[RelativePath] || this._watched[FullPath];

    if (!this._throttle('remove', RelativePath, 100)) return ;
    var watchedDirs = Object.keys(this._watched);
    if (!isDirectory && !this.options.useFsEvents && watchedDirs.length === 1) {
      this.add(Directory, ItemPath, true);
    }
    var nestedDirectoryChildren = this._getWatchedDir(RelativePath).children();
    nestedDirectoryChildren.forEach(function(nestedItem) {
      this._remove(ItemPath, nestedItem);
    }, this);
  }
}