...
FSWatcher.prototype._getWatchHelpers = function(path) {
  ...
  var entryPath = function(entry) {
    return sysPath.join(watchPath, entry.path);
  }

  var globstarFilter = function(entry) {
    return globstarRoot && // globstar is in use
      // and beginning parts of both paths match
      !sysPath.resolve(entryPath(entry))
        .indexOf(sysPath.resolve(globstarRoot))
  }

  var filterPath = function(entry) {
    return (!globFilter || globFilter(entryPath(entry))) &&
...
// * paths     - array of strings (path or directory paths).
// * _origAdd  - private argument for handling non-existent paths to be watched
// * _internal - private argument indicating a non-user add
...
FSWatcher.prototype.add = function(paths, _origAdd, _internal) {
  ...
  paths = paths.filter(function(path) {
    if (path[0] === '!') this._globIgnored.push(path.substring(1));
    else return true;
  }, this);
  ...
  if (this.options.useFsEvents && FsEventsHandler.canUse()) {
    ...
    each(paths, function(path, next) {
      this._addToNodeFs(path, !_internal, 0, _origAdd, function(err, res) {
...
// Public: Remove all listeners from watched paths.
