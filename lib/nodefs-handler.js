...
function(dir, stats, initialAdd, depth, target, callback) {
  var _this = this;
...
function(path, initialAdd, depth, target, callback) {
  ...
  fs[followSymlinks ? 'stat' : 'lstat'](path, function(error, stats) {
    if (this._handleError(error)) return callback(null, path);
    var permError = this.options.ignorePermissionErrors &&
      !this._hasReadPermissions(stats);
    if (permError || this._isIgnored(path, stats)) {
      this._emitReady();
      return callback(null, false);
    }
    if (stats.isDirectory()) {
      this._handleDir(path, stats, initialAdd, depth, target, this._emitReady);
    } else if (stats.isSymbolicLink()) {
      var parent = sysPath.dirname(path);
      this._getWatchedDir(parent).add(path);
      this._emit('add', path, stats);
      this._handleDir(parent, stats, initialAdd, depth, path, this._emitReady);
      fs.realpath(path, function(error, linkPath) {
        this._symlinkPaths[sysPath.resolve(path)] = linkPath;
        this._emitReady();
      }.bind(this));
    } else {
      this._handleFile(path, stats, initialAdd, this._emitReady);
    }
    callback(null, false);
  }.bind(this));
};
