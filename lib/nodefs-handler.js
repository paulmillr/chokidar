...
// Private: Handle symlinks encountered while reading a dir

// * entry      - entry object returned by readdirp
// * directory  - path of the directory being read
// * path       - path of this item
// * item       - basename of this item
...
// * wh         - common watch helpers for this path
  ...
  var read = function(directory, initialAdd, done) {
    ...
    readdirp({
      ...
      if (item === target || !target && !previous.has(item)) {
        this._readyCount++;
        if (this.options.atomic && /\~$/.test(item)) {
          this._emit('change', item.slice(0, -1), entry.stat);
        }
