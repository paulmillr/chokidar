...
function(dir, stats, initialAdd, target, depth, callback) {
  var _this = this;
...
function(item, initialAdd, depth, target, callback) {
  ...
  if (this._isIgnored(item) || this.closed || depth > this.options.depth + 1) {
    this._emitReady();
  ...
  fs[followSymlinks ? 'stat' : 'lstat'](item, function(error, stats) {
    ...
    if (stats.isDirectory()) {
      this._handleDir(item, stats, initialAdd, target, depth, this._emitReady);
    } else if (stats.isSymbolicLink()) {
      var parent = sysPath.dirname(item);
      this._getWatchedDir(parent).add(item);
      this._emit('add', item, stats);
      this._handleDir(parent, stats, initialAdd, item, depth, this._emitReady);
