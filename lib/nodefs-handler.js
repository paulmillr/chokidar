...
// * callback   - called after add event is emitted
...
// * stats      - object, result of fs.stat
// * initialAdd - boolean, was the file added at watch instantiation?
// * depth      - depth relative to user-supplied path
// * target     - child path actually targeted for watch
// * globFilter - filter function for globbed watch definition
// * callback   - called when dir scan is complete

// Returns nothing.
NodeFsHandler.prototype._handleDir =
function(dir, stats, initialAdd, depth, target, globFilter, callback) {
  var _this = this;
  function read(directory, initialAdd, done) {
    ...
    var current = [];
    var filter = globFilter && function(entry) {
      return globFilter(sysPath.join(directory, entry.path));
    };
    readdirp({
      root: directory,
      entryType: 'all',
      fileFilter: filter,
      directoryFilter: globFilter,
      depth: 0,
      lstat: true
  ...
  if (!(initialAdd && this.options.ignoreInitial) && !target && !globFilter) {
...
// * depth      - depth relative to user-supplied path
// * target     - child path actually targeted for watch
// * callback   - indicates whether the path was found or not

// Returns nothing.
NodeFsHandler.prototype._addToNodeFs =
function(path, initialAdd, depth, target, callback) {
  ...
  var globFilter = watchPath !== path ? anymatch(path) : false;
  var followSymlinks = this.options.followSymlinks;
  fs[followSymlinks ? 'stat' : 'lstat'](watchPath, function(error, stats) {
    if (this._handleError(error)) return callback(null, path);
    var permError = this.options.ignorePermissionErrors &&
      !this._hasReadPermissions(stats);
    if (permError || this._isIgnored(watchPath, stats)) {
      this._emitReady();
      return callback(null, false);
    }
    if (stats.isDirectory()) {
      ...
      var parent = sysPath.dirname(watchPath);
      this._getWatchedDir(parent).add(watchPath);
      this._emit('add', watchPath, stats);
      this._handleDir(
        parent,
        stats,
        initialAdd,
      ...
      fs.realpath(path, function(error, linkPath) {
        this._symlinkPaths[sysPath.resolve(path)] = linkPath;
        this._emitReady();
      }.bind(this));
    } else {
      this._handleFile(watchPath, stats, initialAdd, this._emitReady);
    }
    callback(null, false);
  }.bind(this));
};
