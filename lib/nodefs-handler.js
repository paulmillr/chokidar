...
function fsWatchBroadcast(fullPath, type, val1, val2, val3) {
  if (!FsWatchInstances.has(fullPath)) return;
  FsWatchInstances.get(fullPath)[type].forEach(function(listener) {
...
_handleFile(file, stats, initialAdd, callback) {
  ...
  const closer = this._watchWithNodeFs(file, function(path, newStats) {
  ...
  fs[wh.statMethod](wh.watchPath, function(error, stats) {
    ...
    if (stats.isDirectory()) {
      ...
      fs.realpath(path, function(error, targetPath) {
        this._symlinkPaths.set(sysPath.resolve(path), targetPath);
        ready();
      }.bind(this));
    } else {
