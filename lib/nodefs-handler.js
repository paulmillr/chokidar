...
function(item, initialAdd, depth, target, callback) {
  ...
  fs[followSymlinks ? 'stat' : 'lstat'](item, function(error, stats) {
    ...
    if (permError || this._isIgnored(item, stats)) {
      this._emitReady();
      return callback(null, false);
    }
    if (stats.isDirectory()) {
      this._handleDir(item, stats, initialAdd, depth, target, this._emitReady);
    } else if (stats.isSymbolicLink()) {
      var parent = sysPath.dirname(item);
      this._getWatchedDir(parent).add(item);
      this._emit('add', item, stats);
      this._handleDir(parent, stats, initialAdd, depth, item, this._emitReady);
      fs.realpath(item, function(error, linkPath) {
        this._symlinkPaths[sysPath.resolve(item)] = linkPath;
        this._emitReady();
      }.bind(this));
    } else {
      this._handleFile(item, stats, initialAdd, this._emitReady);
    }
    callback(null, false);
  }.bind(this));
};
