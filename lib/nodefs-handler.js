 ...
 * @property {Array} errHandlers
 * @property {Array} rawEmitters
...
const setFsWatchListener = (path, fullPath, options, handlers) => {
  ...
  if (cont) {
    cont.listeners.push(listener);
    cont.errHandlers.push(errHandler);
    cont.rawEmitters.push(rawEmitter);
  } else {
    watcher = createFsWatchInstance(
      path,
      options,
  ...
  return function close() {
    cont.listeners.splice(index, 1);
    cont.errHandlers.splice(index, 1);
    cont.rawEmitters.splice(index, 1);
    if (!cont.listeners.length) {
      // Check to protect against issue gh-730.
      if (!cont.watcherUnusable) cont.watcher.close();
      FsWatchInstances.delete(fullPath);
      cont.listeners = cont.errHandlers = cont.rawEmitters = emptyArr;
      cont.watcher = null;
      Object.freeze(cont);
...
const setFsWatchFileListener = (path, fullPath, options, handlers) => {
  ...
  const copts = container && container.options;
    ...
    container = {
      ...
      watcher: fs.watchFile(fullPath, options, (curr, prev) => {
        ...
        if (curr.size !== prev.size || currmtime > prev.mtimeMs || currmtime === 0) {
          container.listeners.forEach((listener) => listener(path, curr));
        }
      })
    };
    FsWatchFileInstances.set(fullPath, container);
  }
  const index = container.listeners.length - 1;

  // Removes this instance's listeners and closes the underlying fs_watchFile
  // instance if there are no more listeners left.
  return function close() {
    container.listeners.splice(index, 1);
    container.rawEmitters.splice(index, 1);
    if (!container.listeners.length) {
      FsWatchFileInstances.delete(fullPath);
      fs.unwatchFile(fullPath);
      Object.freeze(container);
      Object.freeze(container.listeners);
    }
  };
};
...
_watchWithNodeFs(path, listener) {
  ...
  let closer;
  if (opts.usePolling) {
    options.interval = this.fsw.enableBinaryInterval && isBinaryPath(basename) ?
    ...
    fs.realpath(path, (error, linkPath) => {
      if (dir.has(item)) {
        if (this.fsw._symlinkPaths.get(full) !== linkPath) {
          ...
          this.fsw._emit('change', path, entry.stat);
        }
      } else {
        dir.add(item);
        this.fsw._symlinkPaths.set(full, linkPath);
        this.fsw._emit('add', path, entry.stat);
      }
      this.fsw._emitReady();
    });
    return true;
...
_addToNodeFs(path, initialAdd, priorWh, depth, target, callback) {
  ...
  fs[wh.statMethod](wh.watchPath, (error, stats) => {
    ...
    if (closer) this.fsw._closers.set(path, closer);
    callback(null, false);
  });
}
