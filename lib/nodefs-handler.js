...
function createFsWatchInstance(path, options, listener, errHandler, emitRaw) {
  const handleEvent = (rawEvent, evPath) => {
    ...
    if (evPath && path !== evPath) {
      fsWatchBroadcast(
        sysPath.resolve(path, evPath), FUNCTIONS[0], sysPath.join(path, evPath)
      );
    }
  };
  try {
...
const setFsWatchListener = (path, fullPath, options, handlers) => {
  const {listener, errHandler, rawEmitter} = handlers.listener;
  ...
  if (cont) {
    addAndConvert(cont, FUNCTIONS[0], listener);
    addAndConvert(cont, FUNCTIONS[1], errHandler);
    addAndConvert(cont, FUNCTIONS[2], rawEmitter);
  } else {
    watcher = createFsWatchInstance(
      path,
      options,
      fsWatchBroadcast.bind(null, fullPath, FUNCTIONS[0]),
      errHandler, // no need to use broadcast here
      fsWatchBroadcast.bind(null, fullPath, FUNCTIONS[2])
    );
    if (!watcher) return;
    const broadcastErr = fsWatchBroadcast.bind(null, fullPath, FUNCTIONS[1]);
  ...
  return () => {
    delFromSet(cont, FUNCTIONS[0], listener);
    delFromSet(cont, FUNCTIONS[1], errHandler);
    delFromSet(cont, FUNCTIONS[2], rawEmitter);
    if (isEmptySet(cont.listeners)) {
      ...
      FUNCTIONS.forEach(clearItem(cont));
      cont.watcher = null;
      Object.freeze(cont);
    }
  };
...
async _handleDir(dir, stats, initialAdd, depth, target, wh, realpath) {
  ...
  if (!(initialAdd && this.fsw.options.ignoreInitial) && !target && !tracked) {
    if (!wh.hasGlob || wh.globFilter(dir)) this.fsw._emit(EV_ADDDIR, dir, stats);
  }
